...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const audioModule = {
    ...
    playPreview: (track) => {
      ...
      const previewSrc = `${ASSET_BASE_URL}/assets/audio/${track.previewUrl}`;
  ...
  const preloadModule = {
    preloadAssets: (tracks) => {
      ...
      tracksToPreload.forEach((track) => {
        if (track.cover) {
          const img = new Image();
          img.src = `${ASSET_BASE_URL}/assets/covers/${track.cover}`;
          img.onload = () => {
            loadedCount++;
            updateProgress();
          };
          img.onerror = () => {
            console.error(`Failed to preload cover: ${ASSET_BASE_URL}/assets/covers/${track.cover}`);
            img.src = `${ASSET_BASE_URL}/assets/covers/fallback.jpg`;
            loadedCount++;
            updateProgress();
          };
        }
        if (track.videoUrl) {
          ...
          video.src = `${ASSET_BASE_URL}/assets/preview/${track.videoUrl}`;
          video.preload = 'auto';
          video.onloadeddata = () => {
            loadedCount++;
            updateProgress();
  ...
  const modalModule = {
    ...
    renderModal: (track) => {
      ...
      if (modalCover) modalCover.src = `${ASSET_BASE_URL}/assets/covers/${cover}`;
  ...
  const videoModule = {
    openVideoPopup: (track, youtubeUrl, instrument) => {
      ...
      if (elements.videoTrackCover) elements.videoTrackCover.src = `${ASSET_BASE_URL}/assets/covers/${track.cover}`;
  ...
  const trackModule = {
    renderTracks: (tracks, clearExisting = true) => {
      ...
      tracks.forEach((track) => {
        ...
        img.onerror = () => {
          console.error(`Failed to load cover: ${ASSET_BASE_URL}/assets/covers/${track.cover}`);
          img.src = `${ASSET_BASE_URL}/assets/covers/fallback.jpg`;
          loadingSpinner.remove();
          img.style.display = '';
          img.classList.add('loaded');
